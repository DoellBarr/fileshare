    ...
    async def start(self):
        ...
        if FORCE_SUB_GROUP:
            try:
                ...
                self.LOGGER(__name__).warning(
                    f"Silakan periksa kembali var FORCE_SUB_GROUP dan Pastikan Bot anda Admin di Grup dengan izin link invite Pengguna melalui link undangan, Subs Channel Saat Ini: {FORCE_SUB_GROUP}"
                )
                self.LOGGER(__name__).info(
                    "\nBot Berhenti. Gabung Group https://t.me/SharingUserbot untuk Bantuan"
                )
